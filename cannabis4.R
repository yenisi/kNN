...
scatterplot <- ggplot(train_data, aes(x = conscientiousness, y = agreeableness, color = cannabis)) +
  geom_point() +
  labs(title = "Scatterplot of conscientiousness vs. agreeableness",
       x = "conscientiousness", y = "agreeableness", color = "cannabis") +
  scale_color_manual(values = c("CL0" = "blue", "CL1" = "green", "CL2" = "red", "CL3" = "orange", "CL4" = "purple")) +
  theme_minimal()

# Train k-NN classifier
k <- 10  # Number of neighbors
knn_model <- knn(train_data[, c("conscientiousness", "agreeableness")], test_data[, c("conscientiousness", "agreeableness")], train_data$cannabis, k)
...
grid <- expand.grid(conscientiousness = x_grid, agreeableness = y_grid)

# Predict class labels for each point in the grid
grid$predicted <- knn(train_data[, c("conscientiousness", "agreeableness")], grid, train_data$cannabis, k)

# Plot decision boundaries
# Create decision boundary plot
decision_boundary <- ggplot(grid, aes(x = conscientiousness, y = agreeableness, color = predicted)) +
  geom_point(size = 1) +
  ...
  labs(title = "Decision Boundaries for k-NN Classification",
       x = "conscientiousness", y = "agreeableness", color = "Predicted cannabis Class") +
  theme_minimal()
