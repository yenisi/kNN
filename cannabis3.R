...
scatterplot <- ggplot(train_data, aes(x = openness_to_experience, y = neuroticism, color = cannabis)) +
  geom_point() +
  labs(title = "Scatterplot of openness_to_experience vs. Neuroticism",
       x = "openness_to_experience", y = "Neuroticism", color = "cannabis") +
  scale_color_manual(values = c("CL0" = "blue", "CL1" = "green", "CL2" = "red", "CL3" = "orange", "CL4" = "purple")) +
  theme_minimal()

# Train k-NN classifier
k <- 10  # Number of neighbors
knn_model <- knn(train_data[, c("openness_to_experience", "neuroticism")], test_data[, c("openness_to_experience", "neuroticism")], train_data$cannabis, k)
...
grid <- expand.grid(openness_to_experience = x_grid, neuroticism = y_grid)

# Predict class labels for each point in the grid
grid$predicted <- knn(train_data[, c("openness_to_experience", "neuroticism")], grid, train_data$cannabis, k)

# Plot decision boundaries
# Create decision boundary plot
decision_boundary <- ggplot(grid, aes(x = openness_to_experience, y = neuroticism, color = predicted)) +
  geom_point(size = 1) +
  ...
  labs(title = "Decision Boundaries for k-NN Classification",
       x = "openness_to_experience", y = "Neuroticism", color = "Predicted cannabis Class") +
  theme_minimal()
