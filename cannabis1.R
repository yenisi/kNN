...
scatterplot <- ggplot(train_data, aes(x = extraversion, y = neuroticism, color = cannabis)) +
  geom_point() +
  labs(title = "Scatterplot of Extraversion vs. Neuroticism",
       x = "Extraversion", y = "Neuroticism", color = "cannabis") +
  scale_color_manual(values = c("CL0" = "blue", "CL1" = "green", "CL2" = "red", "CL3" = "orange", "CL4" = "purple")) +
  theme_minimal()

# Train k-NN classifier
k <- 10  # Number of neighbors
knn_model <- knn(train_data[, c("extraversion", "neuroticism")], test_data[, c("extraversion", "neuroticism")], train_data$cannabis, k)
...
grid$predicted <- knn(train_data[, c("extraversion", "neuroticism")], grid, train_data$cannabis, k)
...
decision_boundary <- ggplot(grid, aes(x = extraversion, y = neuroticism, color = predicted)) +
  ...
  labs(title = "Decision Boundaries for k-NN Classification",
       x = "Extraversion", y = "Neuroticism", color = "Predicted cannabis Class") +
  theme_minimal()
...
#---cannabis
#----"impulsivity", "openness_to_experience"
...
# Create scatterplot with color coded points and legend
scatterplot <- ggplot(train_data, aes(x = openness_to_experience, y = agreeableness, color = cannabis)) +
  geom_point() +
  labs(title = "Scatterplot of openness_to_experience vs. agreeableness",
       x = "openness_to_experience", y = "agreeableness", color = "cannabis") +
  scale_color_manual(values = c("CL0" = "blue", "CL1" = "green", "CL2" = "red", "CL3" = "orange", "CL4" = "purple")) +
  theme_minimal()

# Train k-NN classifier
k <- 10  # Number of neighbors
knn_model <- knn(train_data[, c("openness_to_experience", "agreeableness")], test_data[, c("openness_to_experience", "agreeableness")], train_data$cannabis, k)
...
# Predict class labels for each point in the grid
grid$predicted <- knn(train_data[, c("openness_to_experience", "agreeableness")], grid, train_data$cannabis, k)
...
decision_boundary <- ggplot(grid, aes(x = openness_to_experience, y = agreeableness, color = predicted)) +
  geom_point(size = 1) +
  scale_color_manual(values = c("CL0" = "lightblue", "CL1" = "green", "CL2" = "red", "CL3" = "orange", "CL4" = "purple")) +
  labs(title = "Decision Boundaries fo
r k-NN Classification",
       x = "openness_to_experience", y = "agreeableness", color = "Predicted cannabis Class") +
  theme_minimal()
